...
def train_dqn(episodes=200):
    ...
    torch.save(agent.policy_net.state_dict(), "results/dqn_cartpole_baseline.pth")
    df = pd.DataFrame({"Episode": range(len(rewards_per_episode)), "Reward": rewards_per_episode})
    df.to_excel("results/dqn_cartpole_baseline.xlsx", index=False)
    plt.plot(rewards_per_episode)
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.title("DQN Baseline Training Performance")
    plt.savefig("results/dqn_cartpole_baseline_plot.png")
    plt.close()
